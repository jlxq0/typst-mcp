...
isolation, bearer downloads, no-store PDF responses, metrics/OTLP, the
KSC/Lenno/Freudenberg templates, and image smoke/soak gates remain implementation work.
Current `main` has already replaced bulk worker frames with parent-staged, path-only job
I/O; deployed `v0.1.8` predates that checkpoint. See `Plan.md` for the ordered checklist.
