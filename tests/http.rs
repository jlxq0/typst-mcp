...
async fn the_big_five_exercises_five_uploaded_jpegs_end_to_end() {
    ...
    // release image, so give this functional integration test 2 GiB. Exact-image smoke
    // remains the authoritative production-memory gate at 1 GiB per worker.
    let server = TestServer::start_with(&[("WORKER_MEMORY_BYTES", "2147483648")]).await;
